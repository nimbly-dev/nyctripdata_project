...
def load_from_google_cloud_storage(*args, **kwargs):
    ...
    pickup_column_name = kwargs['configuration'].get('pickup_date_column_name')
    ...
    # Generate prefix for the month
    prefix = f'yellow_taxi_trip_data/{pickup_column_name}={year}-{month:02d}'
    ...
    pattern = re.compile(r'yellow_taxi_trip_data/tpep_pickup_datetime=\d{4}-\d{2}-\d{2}/.*\.parquet$')
    ...
    if data_frames:
        ...
        return pd.DataFrame(columns=yellow_taxi_columns)  # Return an empty DataFrame with the correct columns if no data is found
