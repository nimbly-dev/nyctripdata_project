...
{# Define the remote query with all required columns and proper escaping #}
{% set remote_query = (
  "SELECT dwid, ''yellow'' AS cab_type, fare_amount, total_amount, trip_distance, ratecode_id, vendor_id, pu_location_id, do_location_id, pickup_datetime, dropoff_datetime, payment_type, NULL AS dispatching_base_num, NULL AS affiliated_base_number " ~
  "FROM public.yellow_cab_tripdata_production_" ~ year_month
) %}

{# Define the column definitions for the dblink alias #}
