...
def export_data_to_file(df: pd.DataFrame, **kwargs) -> None:
    ...
    with Postgres.with_config(ConfigFileLoader(config_path, config_profile)) as loader:
        ...
        query_to_get_ratecode_summary = "SELECT * FROM dim.dim_rate_code_summary_mv;"
        
        # Load the query results into a DataFrame
        ratecode_summary_df = loader.load(query_to_get_ratecode_summary)
    
    # Define the output file path
    filepath = '/home/src/de_zoomcamp_nyc_taxi/resources/csv/dim_rate_code_summary.csv'
    
    # Export the DataFrame to CSV
    ratecode_summary_df.to_csv(filepath, index=False)
