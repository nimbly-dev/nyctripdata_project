...
def export_data(data, *args, **kwargs):
    ...
    staging_table_name = 'yellow_cab_tripdata_staging'  # Specify the name of the table to export data to
    ...
    appname = f'load_dev_yellow_taxi_data_to_stage_psql'
    # Initialize Spark session
    spark = get_spark_session(
        mode=spark_mode,
        additional_configs=additional_configs, 
        appname="load_dev_yellow_taxi_data_to_stage_psql"
    )
    ...
    with Postgres.with_config(ConfigFileLoader(config_path, config_profile)) as loader:
        ...
        while current_date <= end_date:
            ...
            if not os.path.exists(partition_path):
                LOG.warning(f"Partition path does not exist: {partition_path}")
