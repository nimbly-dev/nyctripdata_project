...
def export_data_to_file(df: pd.DataFrame, **kwargs) -> None:
    ...
    with Postgres.with_config(ConfigFileLoader(config_path, config_profile)) as loader:
        ...
        query_to_get_pickup_dropoff_heatmap = "SELECT * FROM dim.dim_pickup_dropoff_heatmap_mv;"
        
        # Load the query results into a DataFrame
        pickup_dropoff_heatmap_df = loader.load(query_to_get_pickup_dropoff_heatmap)
    
    # Define the output file path
    filepath = '/home/src/de_zoomcamp_nyc_taxi/resources/csv/dim_pickup_dropoff_heatmap_mv.csv'
    
    # Export the DataFrame to CSV
    pickup_dropoff_heatmap_df.to_csv(filepath, index=False)
